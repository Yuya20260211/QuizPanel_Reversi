...
class PresenterWindow(QMainWindow):
    ...
    def __init__(self, state, parent=None):
        ...
        # Initialize UI Grid, Score Rows & Player Buttons
        self.score_text_labels: list[tuple] = []
        self.init_board_grid()
        self.init_score_rows()
    ...
    def init_score_rows(self):
        """Builds score row widgets once; update_ui only updates the text."""
        self.score_text_labels.clear()
        for p in self.state.players:
            ...
            color_box.setStyleSheet(
                f"background-color: {p_color}; border: 1px solid white; border-radius: 3px;"
            )

            text_lbl = QLabel(f"{p_name}: 0枚", self)
            ...
            self.score_text_labels.append((text_lbl, p))
    ...
    def update_ui(self):
        ...
        # 3. Update scores display on right panel (widgets built once in init_score_rows)
        scores = self.state.get_scores()
        for text_lbl, p in self.score_text_labels:
            count = scores.get(p["color"], 0)
            text_lbl.setText(f"{p['name']}: {count}枚")
    ...
    def toggle_contestant_score(self, checked):
        ...
        self.state.show_score_on_contestant = (checked == Qt.CheckState.Checked)
        self.autosave_json()
        self.update_ui()
    ...
    def toggle_contestant_genre(self, checked):
        ...
        self.state.hide_genre_on_contestant = (checked == Qt.CheckState.Checked)
        self.autosave_json()
        self.update_ui()
    ...
    def toggle_answer_always(self, checked):
        self.state.show_answer_always = (checked == Qt.CheckState.Checked)
